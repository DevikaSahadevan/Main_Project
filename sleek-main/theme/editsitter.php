...
if (isset($_POST['update_sitter'])) {
    $sitter_id = $_POST['sitter_id'];
    $asitterid = $_POST['update_sitter'];
    $editquery = mysql_query("UPDATE tbl_sitterassign SET sitterid = '$sitter_id' WHERE asitterid = '$asitterid'");
    if ($editquery) {
        echo '<script>alert("Updated");</script>';
        echo '<script>window.location.href="assignsitter.php"</script>';
    }
}
?>
<!DOCTYPE html>
            ...
            <div class="content-wrapper">
                <div class="content">
                    ...
                    if (isset($_POST['submit_edit'])) {
                         $edit_sitter = $_POST['submit_edit'];

                        $sitterlist = mysql_query("SELECT * FROM tbl_sitterassign WHERE asitterid = '$edit_sitter'");
                        $sitter = mysql_fetch_assoc($sitterlist);
                    }
                                    ...
                                    <div class="col-sm-12 mb-3">
                                        ...
                                        <select name="sitter_id" id="sitter_id" class="form-control" required>
                                            ...
                                            while ($row = mysql_fetch_assoc($query_run)) {
                                                ...
                                                ?>
                                                <option value="<?php echo $row['sitter_id'] ?>"><?php echo $row['name'] ?>
                                                </option>
                                                <?php
                                            }
                                            ?>
                                        ...
                                        <button class="btn btn-primary" name="update_sitter"
                                            value="<?php echo $sitter['asitterid'] ?>" type="submit">Update</button>
                                    </div>
                                </div>
                            </form>
                        </div>
